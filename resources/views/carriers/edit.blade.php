...
@section('title', 'تعديل ناقل')
@section('page_title', 'تعديل ناقل')
@section('page_subtitle', 'تحديث بيانات الناقل')

@section('content')
<link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700&display=swap" rel="stylesheet">
<style>
    :root { --primary-color:#2563eb; --cancel-color:#fef2f2; --cancel-text:#991b1b; --border-soft:#e2e8f0; }
    .modern-rtl-container{direction:rtl;font-family:'Tajawal',sans-serif;max-width:800px;margin:1rem auto;padding:0 20px;}
...
<div class="modern-rtl-container">
    <div class="main-card">
        <div class="card-header-soft">
            <h4 class="fw-bold mb-1">تعديل الناقل</h4>
            <p class="text-muted small mb-0">تحديث بيانات الناقل</p>
        </div>
        ...
        <form method="POST" action="{{ route('carriers.update', $carrier->id) }}">
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="small fw-bold mb-2 d-block">اسم الناقل</label>
                        <input type="text" name="name" class="form-control-soft" value="{{ old('name', $carrier->name) }}" required>
                    </div>
                    <div class="col-md-6">
                        <label class="small fw-bold mb-2 d-block">رقم الجوال</label>
                        <input type="text" name="phone" class="form-control-soft" value="{{ old('phone', $carrier->phone) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="small fw-bold mb-2 d-block">الشخص المسؤول</label>
                        <input type="text" name="contact_person" class="form-control-soft" value="{{ old('contact_person', $carrier->contact_person) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="small fw-bold mb-2 d-block">الحالة</label>
                        <select name="is_active" class="form-control-soft">
                            <option value="1" {{ old('is_active', $carrier->is_active) == 1 ? 'selected' : '' }}>نشط</option>
                            <option value="0" {{ old('is_active', $carrier->is_active) == 0 ? 'selected' : '' }}>غير نشط</option>
                        </select>
                    </div>
                    <div class="col-12">
                        <label class="small fw-bold mb-2 d-block">ملاحظات</label>
                        <textarea name="notes" class="form-control-soft" rows="3">{{ old('notes', $carrier->notes) }}</textarea>
                    </div>
                </div>
            </div>

            <div class="actions-bar">
                <button type="submit" class="btn-modern btn-save">تحديث الناقل</button>
                <a href="{{ route('carriers.index') }}" class="btn-modern btn-cancel">إلغاء</a>
            </div>
        </form>
    </div>
</div>
